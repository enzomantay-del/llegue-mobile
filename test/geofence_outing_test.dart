...
void main() {
  ...
  test('destino en extraPlaces (pending) entra aunque no esté en familyPlaces', () {
    final home = {
      'id': 'home-1',
      'name': 'Casa',
      'lat': -27.0,
    ...
    final watched = placesForKidGeofence(
      familyPlaces: [home],
      extraPlaces: [pendingDest],
      activeTrip: {
        'id': 'trip-1',
        'destinationPlaceId': 'dest-1',
        'destinationName': 'Modista',
      },
    );
    final dest = watched.firstWhere((p) => p['id'] == 'dest-1');
    expect(dest['lat'], -27.3);
    expect(dest['radiusM'], 90);
    expect(dest['status'], 'active');
  });

  test('destino activo ya listado sigue en la geocerca (no era solo Casa)', () {
    final home = {
      'id': 'home-1',
      'name': 'Casa',
      'type': 'home',
    ...
    final withoutTrip = placesForKidGeofence(
      familyPlaces: [home, dest],
      activeTrip: null,
    );
    expect(withoutTrip.map((p) => p['id']).toSet(), {'home-1', 'dest-1'});

    final withTrip = placesForKidGeofence(
      familyPlaces: [home, dest],
      activeTrip: {'id': 'trip-1', 'destinationPlaceId': 'dest-1'},
    );
    expect(withTrip.map((p) => p['id']).toSet(), {'home-1', 'dest-1'});
  });
  ...
  test('POST de llegada/salida de geofence manda tripId y placeId', () {
    ...
    expect(src.contains('extraPlaces: pendingPlaces'), isTrue);
  });
}
